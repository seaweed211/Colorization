...
tf.compat.v1.disable_eager_execution()  # 关闭2.x的即时执行

def the_current_time():
	print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(time.time()))))

CONTENT_IMG =  "inputs/in1.jpg"
STYLE_IMG = "styles/ref1.jpg"
OUTPUT_DIR = "outputs/"
...
def load_image(path):
    ...
    # 替换为PIL的Image.resize，实现和原代码一致的尺寸调整
    image = Image.fromarray(image)
    image = image.resize((IMAGE_W, IMAGE_H))  # 注意顺序是(宽, 高)
    image = np.array(image)  # 转回numpy数组
    image = np.reshape(image, ((1,) + image.shape))
    image = image - MEAN_VALUES
    return image
...
def save_image(path, image):
    ...
    image = np.clip(image, 0, 255).astype('uint8')
    imageio.imwrite(path, image)  # 替代scipy.misc.imsave
...
with tf.compat.v1.Session() as sess:
	...
	# 训练完成后，仅保存最终结果
	the_current_time()
	final_image = sess.run(model['input'])
	final_loss = sess.run(total_loss)
	print('训练完成！最终损失值: ', final_loss)
	# 保存最终图片（命名为final_output.jpg）
	save_image(os.path.join(OUTPUT_DIR, 'output1.jpg'), final_image)
	print('最终图片已保存至：', os.path.join(OUTPUT_DIR, 'output1.jpg'))