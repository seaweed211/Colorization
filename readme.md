...
基于 **MediaPipe** 几何分割、**CLIP** 语义对齐与 **VGG** 风格迁移的灰度图像上色框架。

## 📖 项目简介

本项目旨在解决传统风格迁移在**灰度图上色 (Colorization)** 任务中的结构丢失与颜色溢出问题。不同于全局风格迁移，本框架采用**区域感知 (Region-Aware)** 策略，将图像解耦为 `Skin` (皮肤/身体)、`Hair` (毛发) 和 `Background` (背景) 三个语义层，并利用 CLIP 模型进行跨模态颜色校准。

**核心特性：**
1.  **几何语义分割**：结合 MediaPipe FaceMesh 与几何启发式算法，自动分离面部、头发与身体（无需训练分割模型）。
2.  **区域感知初始化**：在优化前通过 AdaIN 统计量对齐，强制将参考图颜色“移植”到原图，解决优化起点的“灰度陷阱”。
3.  **CLIP 语义引导**：利用 CLIP 视觉-语言模型，自动为每个区域匹配最佳颜色描述（如 "Blue Skin", "Black Hair"）。
4.  **亮度保持融合**：最终输出采用 YUV 空间融合，完美保留原图的高清纹理细节。
